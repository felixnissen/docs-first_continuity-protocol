...
Status: DFC-0001 candidate mapping complete. The ledger remains the traceability
authority for the first specification; later protocol changes require explicit
decisions rather than silent remapping.
...
| `docs/TASK_WORKFLOW.md` "States" | Task lifecycle state is explicit and Draft/Ready/In Progress/terminal transitions have distinct meaning | CORE | C-09 | Makes the baseline transition model explicit |
...
| `docs/concepts_sandbox/README.md` | Undecided material has a named non-authority home | CORE | C-13 | Verbatim intent |
| `docs/concepts_sandbox/README.md` | Material in the sandbox is never authority for active work | CORE | C-14 | Split from the same baseline boundary so each obligation has one stable requirement |
...
| `AGENTS.md` "Addressing and Discoverability" | Present-tense surfaces are validated; history and charters warn | CORE | C-16 | Verbatim intent |
...
The ledger now contains thirty classified rows. The increase from the bootstrap
count of twenty-eight is traceability refinement, not scope expansion: the
baseline task-state model is now represented explicitly for C-09, and the
sandbox boundary was split into its two independently checkable obligations
(C-13 and C-14).
...
| Present-tense validation, tense-aware exemptions (C-16) | The check failed a charter that named its own deliverable | Hours |
